...
for s = 1:length(sbj)
    ...
    for t = 5:5%length(elencoTrial)
        ...
        for r = 1:length(repMap)
            rep = Datastr.cutMovements.ID.L5_S1_Flex_Ext_moment.(repMap{r});
...
load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\SQ_noExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%    plot(SQ_noExo(i,:))
...
for s = 1:7
    SQ_noExo_meanSBJ(s,:) = mean(SQ_noExo((s-1)*10+1:(s)*10,:));
end
figure
SQ_noExo_mean = mean(SQ_noExo);

SQ_noExo_std = std(SQ_noExo);
 hold on
SQ_noExo_mean_SBJ = mean(SQ_noExo_meanSBJ);
...
hold on
plot(SQ_noExo_mean_SBJ, 'm',linewidth=2)

%% SQ yes Exo

load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\SQ_yesExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%     plot(SQ_yesExo(i,:))
% end
figure
% SQ_yesExo_mean = mean([SQ_yesExo(1:10,:);SQ_yesExo(21:30,:)]/98.1);
% 
SQ_yesExo_mean = mean([SQ_yesExo(1:30,:);SQ_yesExo(32:end,:)]);
SQ_yesExo_std = std([SQ_yesExo(1:30,:);SQ_yesExo(32:end,:)]);
 hold on
...
load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\ST_noExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%    plot(ST_noExo(i,:))
% end
ST_noExo_mean = mean([ST_noExo(1:20,:);ST_noExo(51:end,:)]); % bella curva media

ST_noExo_mean = mean([ST_noExo(1:30,:);ST_noExo(32:end,:)]);
ST_noExo_std = std([ST_noExo(1:30,:);ST_noExo(32:end,:)]);
figure
...
load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\ST_yesExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%     plot(ST_yesExo(i,:))
...
ST_yesExo_mean = mean(ST_yesExo);
ST_yesExo_std = std(ST_yesExo);
figure 
hold on
...
load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\BL_noExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%    plot(BL_noExo(i,:))
...
load('C:\Users\Dottorato\OneDrive - University of Twente\Data\MATLAB\BL_yesExo_L5S1Torque.mat')
% figure
% for i=1:70
%     hold on
%     plot(BL_yesExo(i,:))
