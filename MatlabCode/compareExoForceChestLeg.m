...
for t = 1:length(movement)
    for j = 1:length(sbj)
        ...
        % import datastr
        namefile = ['C:\Twente\IUVO\', sbj{j}, '\DatastrMARTA\', sbj{j}, '_', condition{c}, '_', movement{t}, '_10.mat']
        load(namefile);
        ...
        nameTorqueFile =   strcat('C:\Twente\IUVO\', sbj{j}, '\OS\DataFiles\ResultOS\ID_', condition{c}, '_', movement{t}, '_10.sto'); 
        ID = importdata(nameTorqueFile);
        data = ID.data(:,18);
        %% Cut samples
        tau = zeros(1,nReps);
        ...
        for m = 1:sz(2)
            ...
            L5S1.(movement{t}).(condition{c}).(sbj{j}) = Repetitions;
        end
