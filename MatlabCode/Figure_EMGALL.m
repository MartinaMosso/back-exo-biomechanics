...
for r = 1:3          % righe: Peak, RMS, iEMG
    for c = 1:3      % colonne: Squat, Stoop, Bilateral
        nexttile; hold on; box on;

        met = rows{r};   % 'Peak'/'RMS'/'iEMG'
        tsk = tasks{c};  % 'Squat'/'Stoop'/'Bilateral'

        A_no = DATA.(tsk).(met).no;  % [n x 3]
        A_ex = DATA.(tsk).(met).ex;  % [n x 3]

        muNo = mean(A_no,1,'omitnan');  sdNo = std(A_no,0,1,'omitnan');
        muEx = mean(A_ex,1,'omitnan');  sdEx = std(A_ex,0,1,'omitnan');
        ...
        x1 = b(1).XEndPoints;  % posizioni NoExo
        x2 = b(2).XEndPoints;  % posizioni Exo
        ...
        set(gca,'XTick',1:3,'XTickLabel',muscles,'FontSize',fsBase);
        if r==2, ylabel('Amplitude (norm. units)','FontSize',fsBase); end % riga centrale
        title(sprintf('(%c) %s', 'A'+(r-1)*3+(c-1), tsk), 'FontSize', fsBase+1, 'FontWeight','bold');

        % Annotazioni di significatività
        pvec = PVAL.(tsk).(met);   % [1x3] p per LT/LL/IL
        yMax = max(MU(:)+SD(:)); yMin = min(MU(:)-SD(:)); yr = max(1e-6,yMax-yMin);
        for m = 1:3
            pv = pvec(m);
            if ~isnan(pv) && pv < 0.05
                yH = max(MU(m,:) + SD(m,:)) + yPadFrac*yr;
                plot([x1(m) x2(m)], [yH yH], 'k-', 'LineWidth', 1);
                text(mean([x1(m), x2(m)]), yH + 0.015*yr, p2ast(pv), ...
...
%% --- SOLO COLONNA "Stoop" IN RIGA (1x3) ---
fig = figure('Color','w','Units','centimeters','Position',[2 2 18.5 6]);
tiledlayout(fig, 1, 3, 'Padding','compact', 'TileSpacing','compact');

tsk = 'Stoop';  % task fissa

for r = 1:3   % Peak, RMS, iEMG
    nexttile; hold on; box on;
    ...
    set(gca,'XTick',1:3,'XTickLabel',muscles,'FontSize',fsBase);
    if r==1, ylabel('Amplitude (norm. units)','FontSize',fsBase); end

    % Titolo (A, B, C)
    title(sprintf('(%c) %s - %s', 'A'+(r-1), tsk, met), ...
          'FontSize', fsBase+1, 'FontWeight','bold');

    % Annotazioni significatività
    pvec = PVAL.(tsk).(met);   
    yMax = max(MU(:)+SD(:)); yMin = min(MU(:)-SD(:)); yr = max(1e-6, yMax - yMin);
    for m = 1:3
        pv = pvec(m);
...
% Esporta grafica vettoriale con sfondo trasparente
copygraphics(gcf, 'BackgroundColor', 'none', 'ContentType', 'vector');

%% ----- ESPORTA -----
set(fig,'Renderer','painters'); % vettoriale
pdfFile = fullfile(outDir,'Fig_EMG_Grid.pdf');
pngFile = fullfile(outDir,'Fig_EMG_Grid.png');
