function [Datastr] = S1_importMatQTM_16EMG(Datastr, matQTMfolder, fSign, ...
    fTrans,eSign,AnaEMGInd)
% gBMPDynUI matQTMfolder=1; fSign=1; fTrans=1; emgSign=1;
...
DatastrRaw.Marker.DataLabel=[];
DatastrRaw.Marker.MarkerData=[];
...
if isfield(Datastr.Info,'ForcePlateNameRight')
    
    ForcePlateNameRight = Datastr.Info.ForcePlateNameRight; % = 'Force-plate Right';
    ForcePlateNameLeft = Datastr.Info.ForcePlateNameLeft; %= 'Force-plate Left';
else
    ForcePlateNameRight  = 'Force-plate Right';
    ForcePlateNameLeft = 'Force-plate Left';
end
...
if strcmp(fSign, 'True')
    ...
    for i = 1:size(QTMData.Force, 2)
        ...
        if strcmp(char(QTMData.Force(i).ForcePlateName), ForcePlateNameRight)
        ...
        elseif strcmp(char(QTMData.Force(i).ForcePlateName), ForcePlateNameLeft)
...
%     AnaEMGInd = Datastr.Info.AnaEMGInd;
