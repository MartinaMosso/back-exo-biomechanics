function [Datastr] = S1_importMatINAIL_old(Datastr, matfolder, taskName, fTrans)
...
if strcmpi(taskName,'Standing')
    ...
    for tCt = 1:taskList
        checkCount = strcmpi(taskName,MATDataFile.Kinematic_Raw_data(tCt).task);
        
        if checkCount == 1 && taskNo ~= 0
            error('Not handled')
        end
    ...
    % Force data:
    MATDataForces = MATDataFile.Kinetic_Raw_data(taskNo);
    ...
    % extract EMG data
    MATDataEMG = MATDataFile.sEMG_Raw_data(taskNo);
