function [Datastr] = S1_importEMGMatQTM(Datastr, matQTMfolder, markerSign, fSign, ...
    fTrans,emgSign)
% gBMPDynUI matQTMfolder=1; markerSign=1; fSign=1; fTrans=1; emgSign=1;
...
%        ----------- -----------
%
% -emgSign: string, specifying whether the emgData are included or not.
%
...
if sum(match) == 1
    ...
    try
        warning(['Skipping Mat file ' subjrootfolder(end-5:end) '+' subjtrial ': not found or multiple found.']);
        return;
    catch
...
if strcmp(emgSign, 'True')
    ...
    DatastrRaw.EMG.ChannelNumbers = AnaEMGInd;
else
    DatastrRaw.EMG = [];
end
...
% Store marker, force, and EMG data into Datastr
