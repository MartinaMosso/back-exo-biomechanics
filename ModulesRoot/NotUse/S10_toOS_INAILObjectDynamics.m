function [Datastr] = S10_toOS_INAILObjectDynamics(Datastr, osInstallPath, osFolder, removeEndFrames, LowPassCutOff, CoPOffset, ObjectName, handMarkerLabels)
% gBMPDynUI osInstallPath = 1; osFolder=1; removeEndFrames=1; LowPassCutOff = 1; CoPOffset = 1; ObjectName = 1; handMarkerLabels = 1;
%
...
% Datastr = getCarriedObjectIKID(Datastr,osInstallPath,osFolder,LowPassCutOff,ObjectName);

boxFilePrefix = Datastr.Info.objectosmodfile;

if contains(trial,'LI3')
    osimName = [boxFilePrefix '20kg.osim'];
else
    osimName = [boxFilePrefix '21kg.osim'];
end

Datastr = getCarriedObjectIKID(Datastr,osInstallPath,osFolder,LowPassCutOff,ObjectName,osimName);
...
if isfield(Datastr.Resample,'Force')  % Create .mot file (kinetic data)
    Datastr = getExternalLoadsAndCarriedObjectForcesMot(Datastr,dataFilepath, ...
        removeEndFrames, CoPOffset, ObjectName, handMarkerLabels, 1);
else
